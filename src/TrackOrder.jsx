...
// URL Backend sesuai file app.js yang kamu upload
...
const TrackOrder = ({ onBack }) => {
  ...
  const handleCheck = async (e) => {
    ...
    try {
      // PERBAIKAN: URL disesuaikan dengan route backend kamu: /api/orders/:code
      const response = await fetch(`${API_URL}/api/orders/${code}`);
      const data = await response.json();

      if (response.ok) {
        setOrderData(data);
      } else {
        setError(data.message || 'Pesanan tidak ditemukan. Cek kembali kode transaksimu.');
      }
    } catch (err) {
      setError('Gagal terhubung ke server. Pastikan koneksi internet lancar.');
    }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-6 font-sans">
      ...
      <div className="w-full max-w-md flex items-center justify-between mb-8 mt-4">
        <button 
            onClick={onBack} 
            className="bg-red-600 text-white px-4 py-2 rounded-xl font-bold shadow-lg hover:bg-red-700 transition flex items-center gap-2 text-sm"
        >
           &larr; Kembali
        </button>
        <h2 className="text-xl font-black text-gray-800">Lacak Pesanan</h2>
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border-t-4 border-red-600">
          <form onSubmit={handleCheck}>
            ...
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 transition font-mono text-gray-800"
                value={code}
              ...
              <button 
                type="submit" 
                disabled={loading}
                className="bg-red-600 text-white px-6 rounded-xl font-bold hover:bg-red-700 transition shadow-md disabled:opacity-50"
              >
                {loading ? '...' : 'Cek'}
              </button>
            </div>
          ...
          {error && (
            <div className="mt-4 bg-red-50 text-red-600 p-3 rounded-xl text-center text-sm font-bold border border-red-100 animate-pulse">
              {error}
            </div>
          )}
        </div>
        ...
        {orderData && (
          <div className="bg-white p-6 rounded-2xl shadow-md animate-fade-in-up">
            ...
            <div className="text-center mb-6 border-b border-gray-100 pb-6">
              <p className="text-xs text-gray-400 font-bold uppercase mb-2">Status Pesanan Saat Ini</p>
            ...
            <div className="mt-8 bg-gray-50 p-4 rounded-xl border border-gray-100">
              <p className="text-xs text-gray-400 font-bold uppercase mb-2">Menu Dipesan</p>
